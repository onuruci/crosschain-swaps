module AtomicSwapFresh::AtomicSwapV5 {
    use std::signer;
    use std::aptos_hash;
    use std::timestamp;
    ...
    public entry fun initiate_swap(
        ...
        let storage = borrow_global_mut<AtomicSwapStorage>(@AtomicSwapFresh);
    ...
    public fun can_complete(hashlock: vector<u8>): bool acquires AtomicSwapStorage {
        if (!table::contains(&borrow_global<AtomicSwapStorage>(@AtomicSwapFresh).swaps, hashlock)) {
            return false;
        };
        let s = table::borrow(&borrow_global<AtomicSwapStorage>(@AtomicSwapFresh).swaps, hashlock);
